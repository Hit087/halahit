    ...
    >
      <Link href={`/products/${product.id}`}>
        <div className="relative aspect-square overflow-hidden bg-canvas">
          ...
          />
          {isOutOfStock && (
            <span className="absolute top-2 right-2 rounded-full bg-text px-3 py-1 text-xs font-bold text-white">
          ...
          >
            <svg
              className={`h-4 w-4 transition ${inWishlist ? "fill-accent text-accent" : "fill-none text-text/50"}`}
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            >
      ...
      <div className="p-4">
        <Link href={`/products/${product.id}`}>
          <h3 className="text-sm sm:text-base font-semibold text-text line-clamp-2 leading-snug transition hover:text-accent">
            {localizedName(product.name, product.nameEn, locale)}
          </h3>
        </Link>

        <p className="mt-1.5 text-base font-bold text-accent">
          {product.price
